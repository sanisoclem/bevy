...
lazy_static! {
    static ref HEX2SPACE: Mat2 = Mat2::from_cols_array(&[3.0f32.sqrt(), 0.0, 3.0f32.sqrt()/2.0, 3.0/2.0]);
    static ref SPACE2HEX: Mat2 = Mat2::from_cols_array(&[3.0f32.sqrt()/3.0, 0.0, -1.0/3.0, 2.0/3.0]);
}
...
impl HexLayout<CubeHexCoord, Vec2> for CubeHexLayout {
    ...
    fn get_neighbors(&self, hex: CubeHexCoord, max_distance: i32) ->  Self::HexCoordIterator {
        Box::new((1..=max_distance).flat_map(move |ring| (0..=ring).flat_map(move |i| {
